...
/*
 * Create a bcast in the primary group. If exclusive is true, only bcast to
 * ranks; otherwise, bcast to the whole primary group excluding ranks.
 */
static int
bcast_create(crt_opcode_t opc, bool exclusive, d_rank_list_t *ranks,
	     crt_rpc_t **rpc)
{
	struct dss_module_info *info = dss_get_module_info();
	crt_opcode_t		opc_full;

	D_ASSERT(!exclusive || ranks != NULL);
	opc_full = DAOS_RPC_OPCODE(opc, DAOS_RSVC_MODULE, DAOS_RSVC_VERSION);
	return crt_corpc_req_create(info->dmi_ctx, NULL /* grp */,
				    ranks, opc_full, NULL /* co_bulk_hdl */,
				    NULL /* priv */,
				    exclusive ? CRT_RPC_FLAG_EXCLUSIVE : 0,
		   ...
		   size_t size)
{
	...
	rc = bcast_create(RSVC_START, ranks != NULL /* exclusive */,
		  ...
		  bool destroy)
{
	...
	rc = bcast_create(RSVC_STOP, ranks != NULL /* exclusive */,
