	    ...
	    struct dc_tx **ptx)
{
	 ...
	 *
	 * XXX: It is temporary solution to set tx_epoch as client side HLC.
...
dc_tx_check(daos_handle_t th, bool check_write, struct dc_tx **ptx)
{
	...
	if (tx->tx_status != TX_OPEN) {
		D_ERROR("TX is not valid for IO.\n");
...
dc_tx_restart(tse_task_t *task)
{
	...
	if (tx->tx_status != TX_OPEN && tx->tx_status != TX_FAILED) {
		...
		/* XXX: The tx_epoch should be reset as zero, but before server
		 *	side HLC based solution ready, we temporarily set it as
		 *	client side HLC.
		 *
		 *	tx->tx_epoch = 0;
		 */
