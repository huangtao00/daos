...
static int
dx_tx_commit_non_cpd(tse_task_t *task, struct dc_tx *tx)
{
	struct dc_tx_sub_req	*dtsr;
	int			 rc;

	/* There should be only one sub request for non-compounded case. */
	D_ASSERT(tx->tx_sub_count == 1);
...
dc_tx_commit_trigger(tse_task_t *task, struct dc_tx *tx)
{
	 ...
	 */
	return dx_tx_commit_non_cpd(task, tx);
