...
migrate_pool_tls_destroy(struct migrate_pool_tls *tls)
{
	...
	if (tls->mpt_clear_conts)
		d_hash_table_destroy_inplace(&tls->mpt_cont_dest_tab,
					     true /* force */);
	if (tls->mpt_done_cond)
		ABT_cond_free(&tls->mpt_done_cond);
...
migrate_pool_tls_put(struct migrate_pool_tls *tls)
{
	...
	if (tls->mpt_fini && tls->mpt_refcount == 1) {
		if (tls->mpt_pool && tls->mpt_done_cond) {
			ABT_mutex_lock(tls->mpt_pool->spc_lock);
			ABT_cond_signal(tls->mpt_done_cond);
			ABT_mutex_unlock(tls->mpt_pool->spc_lock);
		}
	}
...
int migrate_pool_tls_create_one(void *data)
{
	...
	rc = ABT_cond_create(&pool_tls->mpt_done_cond);
	if (rc != ABT_SUCCESS)
...
migrate_fini_one_ult(void *data)
{
	...
	if (tls->mpt_pool) {
		ABT_mutex_lock(tls->mpt_pool->spc_lock);
		ABT_cond_wait(tls->mpt_done_cond, tls->mpt_pool->spc_lock);
		ABT_mutex_unlock(tls->mpt_pool->spc_lock);
	}
	migrate_pool_tls_put(tls); /* destroy */
